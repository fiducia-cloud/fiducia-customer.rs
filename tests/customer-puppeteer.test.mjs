...
test(
  "puppeteer drives the customer portal login, CSRF boundary, and live streams",
  { timeout: 180_000 },
  async (t) => {
    ...
    t.after(async () => {
      await captureBrowserEvidence("puppeteer", page, browserErrors);
    ...
    // Keep Puppeteer's security-header assertions in parity with Playwright so
    // a driver-specific omission cannot make the suite falsely green.

...
mod tests {
    ...
    const MIGRATION: &str = include_str!(
        "../../fiducia-interfaces/sql/migrations/20260802_customer_shared_auth_session_assurance.sql"
    );
    ...
    async fn database() -> Option<DatabaseConnection> {
        ...
        let db = Database::connect(url)
            .await
            .expect("connect customer session assurance test database");
        let mut ready = SCHEMA_READY.lock().await;
        if !*ready {
            db.execute_unprepared(SCHEMA)
                .await
                .expect("apply canonical customer schema");
            db.execute_unprepared(MIGRATION)
                .await
                .expect("apply customer session assurance migration");
            *ready = true;
        }
        drop(ready);
        Some(db)
    ...
    fn fixed(time: chrono::DateTime<Utc>) -> DateTimeWithTimeZone {
        time.fixed_offset()
    }

    #[test]
    fn binding_input_is_bounded_and_time_ordered() {
        let now = fixed(Utc::now());
        assert!(normalize_binding("Safari on Mac", Some("New York"), now, now + Duration::minutes(5))
            .is_ok());
        assert!(normalize_binding(" ", None, now, now + Duration::minutes(5)).is_err());
        assert!(normalize_binding(
            &"x".repeat(MAX_DEVICE_CHARS + 1),
            None,
            now,
            now + Duration::minutes(5),
        )
        .is_err());
        assert!(normalize_binding("Safari", Some("bad\nlocation"), now, now + Duration::minutes(5))
            .is_err());
        assert!(normalize_binding("Safari", None, now, now).is_err());
    }

    #[tokio::test]
    async fn shared_auth_assurance_is_user_scoped_expiring_unique_and_revocable() {
        let Some(db) = database().await else {
            eprintln!("skip Shared Auth assurance store: TEST_DATABASE_URL unset");
            return;
        };

        let my_subject = Uuid::new_v4();
        let other_subject = Uuid::new_v4();
        let my_user = crate::store::ensure_user(&db, my_subject, "assured@example.invalid")
            .await
            .unwrap();
        crate::store::ensure_user(&db, other_subject, "other@example.invalid")
            .await
            .unwrap();

        let sid = Uuid::new_v4();
        let verified_at = fixed(Utc::now());
        let expires_at = verified_at + Duration::minutes(15);
        let bound = bind_verified_session(
            &db,
            my_user,
            sid,
            "Safari on MacBook",
        ...
        .await
        .unwrap();
        assert_eq!(bound.user_id, my_user);
        assert_eq!(bound.shared_auth_session_id, sid);
        assert_eq!(bound.assurance_verified_at, verified_at);
        assert_eq!(bound.expires_at, expires_at);

        let accepted = verified_session_for_provider(
            &db,
            my_subject,
            sid,
            verified_at + Duration::seconds(1),
        )
        .await
        .unwrap()
        .expect("bound current SID is accepted");
        assert_eq!(accepted.id, bound.id);

        assert!(verified_session_for_provider(
            &db,
            other_subject,
            sid,
            verified_at + Duration::seconds(1),
        ...
        assert!(!revoke_verified_session_for_provider(&db, other_subject, sid)
            .await
            .unwrap());
        assert!(revoke_verified_session_for_provider(&db, my_subject, sid)
            .await
            .unwrap());
        assert!(!revoke_verified_session_for_provider(&db, my_subject, sid)
            .await
